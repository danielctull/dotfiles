...
function _swift_run
{
    ...
    if [[ $COMP_CWORD == $1 ]]; then
        COMPREPLY=( $(compgen -W "-Xcc -Xswiftc -Xlinker -Xcxx --configuration -c --build-path --chdir -C --package-path --sanitize --disable-prefetching --skip-update --disable-sandbox --disable-package-manifest-caching --version --destination --verbose -v --no-static-swift-stdlib --static-swift-stdlib --enable-llbuild-library --force-resolved-versions --disable-automatic-resolution --enable-index-store --disable-index-store --enable-pubgrub-resolver --enable-parseable-module-interfaces --skip-build --build-tests --repl" -- $cur) )
        return
    fi
    case $prev in
        (-Xcc)
...
{
    if [[ $COMP_CWORD == $1 ]]; then
        COMPREPLY=( $(compgen -W "resolve show-dependencies tools-version generate-xcodeproj dump-package init unedit update clean reset describe edit fetch config completion-tool -Xcc -Xswiftc -Xlinker -Xcxx --configuration -c --build-path --chdir -C --package-path --sanitize --disable-prefetching --skip-update --disable-sandbox --disable-package-manifest-caching --version --destination --verbose -v --no-static-swift-stdlib --static-swift-stdlib --enable-llbuild-library --force-resolved-versions --disable-automatic-resolution --enable-index-store --disable-index-store --enable-pubgrub-resolver --enable-parseable-module-interfaces" -- $cur) )
        return
    fi
    case $prev in
        (-Xcc)
...
{
    if [[ $COMP_CWORD == $1 ]]; then
        COMPREPLY=( $(compgen -W "unset-mirror set-mirror get-mirror" -- $cur) )
        return
    fi
    case $prev in
    esac
...
{
    if [[ $COMP_CWORD == $1 ]]; then
        COMPREPLY=( $(compgen -W "-Xcc -Xswiftc -Xlinker -Xcxx --configuration -c --build-path --chdir -C --package-path --sanitize --disable-prefetching --skip-update --disable-sandbox --disable-package-manifest-caching --version --destination --verbose -v --no-static-swift-stdlib --static-swift-stdlib --enable-llbuild-library --force-resolved-versions --disable-automatic-resolution --enable-index-store --disable-index-store --enable-pubgrub-resolver --enable-parseable-module-interfaces --skip-build --list-tests -l --generate-linuxmain --parallel --num-workers --specifier -s --xunit-output --filter --enable-code-coverage" -- $cur) )
        return
    fi
    case $prev in
        (-Xcc)
